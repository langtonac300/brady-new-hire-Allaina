...
- ~~A guided Sheets workbook for hands-on practice doesn't exist yet.~~ **Built 2026-09-06**
  — `06-tracker/sheets-practice/` (data + nine tasks + a generated answer key), paired with
  `02-learning/sheets-for-paid-search.md`. The data and `ANSWERS.md` are both generated by
  `generate.mjs` from one seed; **regenerate and commit them together** or the answer key
  silently stops matching the data.
